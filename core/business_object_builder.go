...
func (builder *businessObjectBuilder) On(action Action, function interface{}) *businessObjectBuilder {
	...
	if typeOfState := reflect.TypeOf(builder.stateEntity.GetInitialState()); functionType.NumIn() != 2 || functionType.NumOut() != 1 || functionType.In(0) != functionType.Out(0) || functionType.In(0) != typeOfState {
...
func (builder *businessObjectBuilder) SetActionsLogicByObject(object interface{}) *businessObjectBuilder {
	...
	for i := 0; i < rt.NumMethod(); i++ {
		m := rt.Method(i)
		mt := m.Type
		if typeOfState := reflect.TypeOf(builder.stateEntity.GetInitialState()); mt.NumIn() == 3 && mt.NumOut() == 1 && mt.In(1) == mt.Out(0) && mt.In(1) == typeOfState {
